...
export const issueBoardColumns = pgTable(
  ...
  (table) => ({
    ...
    companyNameIdx: uniqueIndex("issue_board_columns_company_custom_name_uq").on(
      table.companyId,
      sql`lower(btrim(${table.name}))`,
    ).where(sql`${table.isSystem} = false`),
    companySystemStatusIdx: uniqueIndex("issue_board_columns_company_system_status_uq")
